...
define config.layers = ['real_bg', 'master', 'fade_layer','transient', 'screens','overlay']
#
## 基础 ##########################################################################
...
define config.version = "Indev-22-1"


## 放置在游戏“关于”屏幕的文本。将文本放在三个引号之间，并在段落之间留一个空行。

define gui.about = _p("""
    制作团队:Reborn制作组(QQ群:684869122,欢迎各位小伙伴的加入)
...
define build.name = "External_tianxi"
...
define config.save_directory = "External_tianxi-1650101547"
...
init python:
    ...
    build.archive("l2ds", "all")

    build.classify("game/**.rpy", "scripts")
    build.classify("game/**.rpyc", "scripts")
    build.classify("game/**.png", "images")
    build.classify("game/**.jpg", "images")
    build.classify("game/**.json", "images")
    build.classify("game/**.moc3", "images")
    build.classify("game/**.can3", "images")
